...
isCorrect :: NFA -> Bool
isCorrect (alph, states, init, transFunc, fin) = correctInit && correctFin
    where
        coorectAlph = alph /= []
        correctStates = states /= []
        correctInit = init `elem` states
        correctFin = fin `intersect` states == fin
        -- correctFunc = undefined

{-
    Просто необходимый комментарий. Я не учился на мехмате, но читал по книжкам.
    Поэтому некоторые детали мог упустить. Так вот:
    ...
    сосостояние он больше не может читать символы. Я немного в растерянности, но сделал функцию
    accept в соответствии с этой идеей.
    
    А вот тут я проверяю функцию некоторого автомата на "полноту области определения".
    На вашем результате     hasCorrectFunc nfa_ex   результат = False.
-}

hasCorrectFunc :: NFA -> IO Bool
hasCorrectFunc nfa = (try (checkFunc nfa) :: IO (Either PatternMatchFail Bool))
    >>= return . checkEither
    where
        checkEither (Right res) = res
        checkEither (Left _) = False
        checkFunc :: NFA -> IO Bool
        checkFunc (alph, states, _, transFunc, _) = 
            evaluate $ postStates `intersect` states == postStates
            where
                postStates = concat $ transFunc `liftM` states `ap` alph

-- в дальнейшем может пригодиться функция whileM,
-- которая собирает результаты в список, пока истинно
-- заданное условие
whileM :: m Bool -> m a -> m [a]
whileM = undefined -- Не понял.
...
check_nfa1 = positive && not negative
    where
        positive = and $ (accept nfa1) `liftM` ["2112", "1211", "21212112212"]
        negative = or $ (accept nfa1) `liftM` ["21212", "12121", "22221221", "abcd"]
...
check_nfa3 = positive && not negative
    where
        positive = and $ (accept nfa3) `liftM` ["1211", "212211", "11", "2121112211"]
        negative = or $ (accept nfa3) `liftM` ["1", "21", "112","1121", "abcd"]
